# PicHub 图片管理网站 - 需求规格说明书（SRS，以代码为准）

> 更新时间：2026-01-03  
> 说明：本文档根据当前仓库实现修订，删除了早期与实现不一致的“React/Spring/MCP Server”等规划描述。
...
## 3. 功能需求（对照课程基本功能 1-11）

> 说明：下述每条需求均为“必须具备”。实现入口以当前 UI 与 API 为准。
...
- 若课程验收必须演示 MCP 协议，可在上述 REST 能力基础上封装 MCP 工具层（不影响核心业务逻辑）。
...
## 7. 验收建议（操作清单）
建议按 `docs/test-report.md` 的“手工验收用例”逐条演示，并录制 5–10 分钟操作视频覆盖：
- 注册/登录
- 上传（含 EXIF 图片）
- 标签（自定义 + AI）
- 条件搜索 + NL 搜索
- 加入轮播并播放
- 编辑（裁剪/调色）保存副本（含标签继承、命名序号）
- 删除
- 手机宽度显示效果
